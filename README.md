...
მონაცემებში საკმაოდ ბევრი გამოტოვებული მნიშვნელობა იყო. თითოეულ სვეტს ინდივიდუალურად მივუდექი, რადგან NaN-ის მიზეზი ყველგან სხვადასხვა იყო:

* **PoolQC, Fence, MiscFeature, Alley** — ამ სვეტებში NaN-ების 90%+ იყო.  სვეტები მთლიანად წავშალე, რადგან მათი ინფორმაციური ღირებულება მინიმალური იყო.
...
**Ordinal Encoding** — ხარისხობრივი სვეტებისთვის, სადაც კატეგორიებს შორის ლოგიკური თანმიმდევრობა არსებობს, გამოვიყენე ხელით შედგენილი mapping-ი:
...
One-Hot Encoding — დანარჩენი კატეგორიული სვეტებისთვის გამოვიყენე pd.get_dummies(). drop_first=True. შემდეგ .align() მეთოდით train და test set-ების სვეტები გავათანასწორე.
...
* max_depth=30 — ძალიან ღრმა ხე, რომელიც training მონაცემებს იზეპირდება. test_rmse კარგი გამოვიდა, მაგრამ cv_rmse_mean უფრო მაღალი იყო — ეს overfitting-ის ნიშანია.
...
1. cv_rmse_mean და test_rmse შორის სხვაობა მინიმალურია (~1,500) — მოდელი არ overfits-ებს
2. cross-validation score კარგია, რაც ნიშნავს სტაბილურ პერფორმანსს სხვადასხვა data split-ზე
3. max_depth=30 მოდელს უკეთესი test_rmse ჰქონდა, მაგრამ ეს სავარაუდოდ კონკრეტული random split-ის გამართლებაა და არა ნამდვილი გაუმჯობესება
